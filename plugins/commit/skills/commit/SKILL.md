...
- 若**没有任何已暂存变更，但存在未暂存变更** → 使用 AskUserQuestion 询问：
  - 问题：暂存区为空，检测到以下未暂存变更，如何处理？（展示文件列表）
  - 选项：
    - **全部 add 后提交**：执行 `git add -u`，再进入第三步
    - **中断**：停止操作
- 若**工作区完全干净（无任何变更）** → 告知用户没有内容可提交，终止
...
使用 AskUserQuestion 工具询问用户是否需要 push：
...
若 `git push` 执行失败（如未设置上游分支），使用 AskUserQuestion 工具询问用户要 push 到哪个远程分支：

- 问题：push 失败，请指定远程和分支
- 选项：
  - **origin 当前分支**：执行 `git push -u origin <当前分支名>`
  - **手动输入**：由用户提供远程和分支（如 `origin main`）

若用户选择**手动输入**，让用户在对话中输入远程和分支，然后执行 `git push -u <remote> <branch>`。
